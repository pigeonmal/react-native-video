...
object VideoDownloadStore {
  ...
  private val context: Context
    get() = NitroModules.applicationContext ?: throw com.twg.video.core.LibraryError.ApplicationContextNotFound
  ...
  private val downloadManager: DownloadManager by lazy {
    val upstreamFactory = CronetDataSource.Factory(NitroFetch.getEngine(), NitroFetch.ioExecutor)
  ...
  fun enqueueDownload(config: NativeVideoConfig, options: VideoDownloadOptions): VideoDownloadTask {
    ...
    val downloadId = if (options.downloadId.isNotBlank()) {
      options.downloadId
    } else {
      buildStableId(uri)
    }
    ...
    if (options.downloadExternalSubtitles && !config.externalSubtitles.isNullOrEmpty()) {
      downloadExternalSubtitles(downloadId, config.externalSubtitles, config.headers ?: emptyMap())
    }
  ...
  fun resolveSourceForOffline(config: NativeVideoConfig, downloadId: String): NativeVideoConfig {
    val targetDownloadId = if (downloadId.isNotBlank()) {
      downloadId
    } else {
  ...
  private fun downloadExternalSubtitles(
    ...
    if (!subtitleFolder.exists() && !subtitleFolder.mkdirs()) {
      throw DownloadError.InvalidConfig("Failed to create subtitle storage folder")
    }
    ...
    subtitles.forEachIndexed { index, subtitle ->
      ...
      response.use { res ->
        if (!res.isSuccessful) {
          throw DownloadError.SubtitleFailed(originalUri, "HTTP ${res.code}")
        }
        val body = res.body ?: throw DownloadError.SubtitleFailed(originalUri, "Empty body")
